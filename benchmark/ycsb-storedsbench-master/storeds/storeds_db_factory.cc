...
#include "log_pmem/log_pmem.cc"
#include "log_pmem/malloc_dram.cc"
#include "log_pmem/pmemobj_pmem.cc"
...
StoredsBase *StoredsDbFactory::GetDB(const char *type, const char *path) {
    ...
    } else if(strcmp(type, "log-pmem") == 0) {
        return new LogPmem(path);
    } else if(strcmp(type, "malloc-dram") == 0) {
        return new MallocDram(path);
    } else if(strcmp(type, "pmemobj-pmem") == 0) {
        return new PmemobjPmem(path);
    } else {
        return NULL;
    }
}